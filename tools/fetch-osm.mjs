...
(
  ...
  way["waterway"~"^(river|stream)$"](${ROAD_BBOX.join(',')});
);
out geom tags;
`;
...
const out = { roads, buildings, waterways };
writeFileSync('public/data/osm.json', JSON.stringify(out));
console.log(`roads: ${roads.length} chains (${km.toFixed(0)} km), buildings: ${buildings.length}, waterways: ${waterways.length}`);
